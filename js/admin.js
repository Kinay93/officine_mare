...
function openManualReservationModal() {
  if (manualReservationModal) manualReservationModal.classList.add("open");
}

function closeManualReservationModal() {
  if (manualReservationModal) manualReservationModal.classList.remove("open");
  if (manualReservationForm) manualReservationForm.reset();
  if (mrTime) mrTime.innerHTML = `<option value="">Seleziona prima data e turno</option>`;
}

function openTablesModal() {
  if (tablesModal) tablesModal.classList.add("open");
}

function closeTablesModal() {
  if (tablesModal) tablesModal.classList.remove("open");
...
function refreshManualReservationSlots() {
  if (!mrTime || !mrDate || !mrTurno) return;
...
function getRuleForDay(dayISO, rules) {
  ...
  if (selected) {
    return {
      lunch: ruleValue(selected.lunch_max_covers),
      dinner: ruleValue(selected.dinner_max_covers)
    };
  }
...
function getMaxCoversForReservation(reservation, calendarMap, rules) {
  ...
  if (service === "dinner") {
    return Number(override?.dinner_max_covers ?? base.dinner);
  }

  return Number(override?.lunch_max_covers ?? base.lunch);
}
...
async function loadReservations() {
  ...
  if (pendingCount) pendingCount.textContent = visibleReservations.filter(x => x.status === "pending").length;
  if (confirmedCount) confirmedCount.textContent = visibleReservations.filter(x => x.status === "confirmed").length;
  if (periodLabel) periodLabel.textContent = getPeriodLabel();
  if (periodCount) periodCount.textContent = applyPeriodFilter(visibleReservations).length;
...
const openManualReservationBtn = document.getElementById("openManualReservationBtn");
const closeManualReservationBtn = document.getElementById("closeManualReservationBtn");
const cancelManualReservationBtn = document.getElementById("cancelManualReservationBtn");

const closeTablesModalBtn = document.getElementById("closeTablesModalBtn");
const cancelTablesModalBtn = document.getElementById("cancelTablesModalBtn");
...
if (cardPeriod) {
  cardPeriod.addEventListener("click", cyclePeriod);
}

if (searchInput) searchInput.addEventListener("input", loadReservations);
...
if (manualReservationModal) {
  manualReservationModal.addEventListener("click", (e) => {
    if (e.target === manualReservationModal) closeManualReservationModal();
  });
}

if (mrDate) mrDate.addEventListener("change", refreshManualReservationSlots);
if (mrTurno) mrTurno.addEventListener("change", refreshManualReservationSlots);
if (manualReservationForm) manualReservationForm.addEventListener("submit", saveManualReservation);
